...
f = open("input/problem_log_630_3.txt", "r")
content = f.read()
...
start = 'Expected label values are [0, 1]'
...
for i in range(len(splited_data)):
    ...
    if "Exception in " in splited_data[i]:
        # print(splited_data[i])

        impt_data = splited_data[i].split("Exception in ")
        genral_data = impt_data[1].replace(" :", "")
        print(genral_data)
        # print(impt_data[1])
        list_of_exceptions.append(genral_data)


#%% save list of exceptions in txt file
